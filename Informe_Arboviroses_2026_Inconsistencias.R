...
AUX <- RS22_2025_Incons_Residencia[,-c(1, 3)]

RS22_2025_TAB_INCON_RESID_Municipios <- AUX %>%
  gt() %>%
  tab_header(title = md("**Qualifica SINAN - Dados de Residência (2025)**"),
  ...
  AUX[which(AUX$COD_IBGE == i), 8] <- count(RS22_2025_SINAN %>% 
                                              filter(ID_MN_RESI == i,  
                                                     ...
                                                     DT_CHIK_S1 < (DT_SIN_PRI +5)
                                                        ...
                                                        filter(ID_MN_RESI == i,
                                                               DT_CHIK_S1 != is.na(RS22_2025_SINAN$DT_SORO),
                                                               DT_CHIK_S1 > (DT_SIN_PRI +5),
                                                               Sys.Date() > (DT_SORO + 20),
...
AUX <- RS22_2025_Incons_Laboratorio[,-c(1, 3)]

RS22_2025_TAB_INCON_LABORATORIO_Municipios <- AUX %>%
  gt() %>%
  tab_header(title = md("**Qualifica SINAN - Dados de Laboratóriais (2025)**"),
             subtitle = "Campos 17 a 30 do SINAN online"
  ) %>%
  tab_options(heading.align = "left",
              column_labels.border.top.color = "black",
  ...
  cols_align(align = "center", 
             columns = c(2:6)) %>%
  cols_label(Municipio = "Município",
             Sorologia_Dengue = "Sorologia DENGUE Coletada < 5 dias",
             Sorologia_Chik = "Sorologia CHIKUNGUNYA Coletada < 5 dias",
             PCR = "Pesquisa Arbovírus Coletada > 5 dias",
             Dengue_S_Resultado = "Resultado Sorologia DENGUE Faltando",
             Chik_S_Resultado = "Resultado Sorologia CHIKUNGUNYA Faltando") %>%
  tab_style(style = cell_text(weight = "bold"),
            locations = cells_column_labels(everything()
            )) %>%
  tab_footnote(footnote = Fonte) %>%
...
AUX <- RS22_2025_Incons_Hospitalizacao[,-c(1, 3)]

RS22_2025_TAB_INCON_HOSPITALIZACAO <- AUX %>%
  gt() %>%
  tab_header(title = md("**Qualifica SINAN - Dados de Hospitalização (2025)**"),
...
RS22_2025_SINAN_Duplic <- RS22_2025_SINAN %>%
  filter(duplicated(RS22_2025_SINAN$NU_NOTIFIC))

AUX$Duplicidade <- NA

for(i in BASE_IBGE[(which(BASE_IBGE$RS == RS)), 2]){
  AUX[which(AUX$COD_IBGE == i), 8] <- count(RS22_2025_SINAN_Duplic %>% 
                                              filter(ID_MN_RESI == i,
                                                     ID_AGRAVO != 8)
  )
}
...
AUX <- RS22_2025_Incons_Investigacao[,-c(1, 3)]

RS22_2025_TAB_INCON_INVESTIGACAO <- AUX %>%
  gt() %>%
  tab_header(title = md("**Qualifica SINAN - Investigação Geral (2025)**"),
